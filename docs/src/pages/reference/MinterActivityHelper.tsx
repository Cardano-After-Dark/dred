...
export default function DocumentItem() {
        ...
        <ReactMarkdown> {
            ...
            <p>
                <b>Instance properties: </b>
                                <a href="#$seeded$addingMintInvariant"><var>$seeded$addingMintInvariant</var></a>, &nbsp;
                    <a href="#$seeded$addingSpendInvariant"><var>$seeded$addingSpendInvariant</var></a>, &nbsp;
                    <a href="#$seeded$CreatingNewSpendDelegate"><var>$seeded$CreatingNewSpendDelegate</var></a>, &nbsp;
                    <a href="#$seeded$forcingNewMintDelegate"><var>$seeded$forcingNewMintDelegate</var></a>, &nbsp;
                    <a href="#mintWithDelegateAuthorizing"><var>mintWithDelegateAuthorizing</var></a>
            </p>
         <p>
            <b>Instance methods: </b>
                    <a href="#addingMintInvariant"><var>addingMintInvariant()</var></a>, &nbsp;
                    <a href="#addingSpendInvariant"><var>addingSpendInvariant()</var></a>, &nbsp;
                    <a href="#CreatingNewSpendDelegate"><var>CreatingNewSpendDelegate()</var></a>, &nbsp;
                    <a href="#CreatingNewSpendDelegate"><var>CreatingNewSpendDelegate()</var></a>, &nbsp;
                    <a href="#forcingNewMintDelegate"><var>forcingNewMintDelegate()</var></a>, &nbsp;
                    <a href="#mintingCharter"><var>mintingCharter()</var></a>
        </p>
    ...
    <div className="prose">
        <a id="$seeded$addingMintInvariant"></a>

            <div>
        <h4 style={{display: "inline-block"}}>   readonly {
              "$seeded$addingMintInvariant"}&nbsp;&nbsp;</h4>
        <ReactMarkdown>
            {
...
<div className="prose">
        <a id="$seeded$addingSpendInvariant"></a>

            <div>
        <h4 style={{display: "inline-block"}}>   readonly {
              "$seeded$addingSpendInvariant"}&nbsp;&nbsp;</h4>
        <ReactMarkdown>
            {
...
<p></p>
        ...
        <ReactMarkdown> {
            "\n\n##### Seeded activity This activity uses the pattern of spending a utxo to provide a uniqueness seed. ##### Activity contains implied seed Creates a SeedActivity based on the provided args, reserving space for a seed to be provided implicitly by a SeedActivity-supporting library function. #### Usage Access the activity-creator as a getter: `$seeded$addingSpendInvariant`\n\nUse the resulting activity-creator in a seed-providing context, such as the delegated-data-controller's `mkTxnCreateRecord({activity, ...})` method.\n"
        }</ReactMarkdown>
            <pre><code lang="typescript">
{"get $seeded$addingSpendInvariant(): "}<a href="#SeedActivity">{"SeedActivity"}</a>{"<(thingWithSeed: "}<a href="#hasSeed">{"hasSeed"}</a>{" | "}<a href="#TxOutputId">{"TxOutputId"}</a>{" | string) => "}<a href="#isActivity">{"isActivity"}</a>{">"}{";"}
            </code></pre>
...
<div className="prose">
        <a id="$seeded$CreatingNewSpendDelegate"></a>

            <div>
        <h4 style={{display: "inline-block"}}>    {
              "$seeded$CreatingNewSpendDelegate"}&nbsp;&nbsp;</h4>
        <ReactMarkdown>
            {
...
<p></p>
        ...
        <ReactMarkdown> {
            "\n\n##### Seeded activity This activity uses the pattern of spending a utxo to provide a uniqueness seed. ##### Activity contains implied seed Creates a SeedActivity based on the provided args, reserving space for a seed to be provided implicitly by a SeedActivity-supporting library function.\n\n#### Usage 1. Call the `$seeded$CreatingNewSpendDelegate({ replacingUut })` method with the indicated (non-seed) details. 2. Use the resulting activity in a seed-providing context, such as the delegated-data-controller's `mkTxnCreateRecord({activity})` method.\n\n"
        }</ReactMarkdown>
            <pre><code lang="typescript">
{"$seeded$CreatingNewSpendDelegate: "}{"(fields: {\n        replacingUut: number[] | undefined;\n    }) => "}<a href="#SeedActivity">{"SeedActivity"}</a>{"<(value: "}<a href="#hasSeed">{"hasSeed"}</a>{", fields: {\n        replacingUut: number[] | undefined;\n    }) => "}<a href="#isActivity">{"isActivity"}</a>{">"}{";"}
            </code></pre>
...
<p></p>
        ...
        <ReactMarkdown> {
            "\n\n##### Seeded activity This activity uses the pattern of spending a utxo to provide a uniqueness seed. ##### Activity contains implied seed Creates a SeedActivity based on the provided args, reserving space for a seed to be provided implicitly by a SeedActivity-supporting library function. #### Usage Access the activity-creator as a getter: `$seeded$forcingNewMintDelegate`\n\nUse the resulting activity-creator in a seed-providing context, such as the delegated-data-controller's `mkTxnCreateRecord({activity, ...})` method.\n"
        }</ReactMarkdown>
            <pre><code lang="typescript">
{"get $seeded$forcingNewMintDelegate(): "}<a href="#SeedActivity">{"SeedActivity"}</a>{"<(thingWithSeed: "}<a href="#hasSeed">{"hasSeed"}</a>{" | "}<a href="#TxOutputId">{"TxOutputId"}</a>{" | string) => "}<a href="#isActivity">{"isActivity"}</a>{">"}{";"}
            </code></pre>
...
<div className="prose">
        <a id="mintWithDelegateAuthorizing"></a>

            <div>
        <h4 style={{display: "inline-block"}}>   readonly {
              "mintWithDelegateAuthorizing"}&nbsp;&nbsp;</h4>
        <ReactMarkdown>
            {
...
{"get mintWithDelegateAuthorizing(): "}{"{\n        redeemer: "}<a href="#UplcData">{"UplcData"}</a>{";\n    }"}{";"}
    ...
    <h3>Instance methods</h3>
        <div className="prose">
        <a id="addingMintInvariant"></a>

            <div>
        <h4 style={{display: "inline-block"}}>    {
              "addingMintInvariant"}()&nbsp;&nbsp;</h4>
        <ReactMarkdown>
            {
                "generates isActivity/redeemer wrapper with UplcData for ***\"CapoMintHelpers::MinterActivity.addingMintInvariant\"***, given a transaction-context (or direct arg) with a ***seed utxo***\n\n"
            }
        </ReactMarkdown>
    </div>
...
<div className="prose">
        ...
        <ReactMarkdown>
            {
                "generates isActivity/redeemer wrapper with UplcData for ***\"CapoMintHelpers::MinterActivity.addingSpendInvariant\"***, given a transaction-context (or direct arg) with a ***seed utxo***\n\n"
            }
        </ReactMarkdown>
    </div>
...
<p></p>
        ...
        <ReactMarkdown> {
            "\n\n##### Seeded activity This activity uses the pattern of spending a utxo to provide a uniqueness seed. - to get a transaction context having the seed needed for this argument, see the `tcxWithSeedUtxo()` method in your contract's off-chain StellarContracts subclass. - or see the {@link hasSeed} type for other ways to feed it with a TxOutputId. - in a context providing an implicit seed utxo, use the `$seeded$addingSpendInvariant}` variant of this activity instead\n"
        }</ReactMarkdown>
            <pre><code lang="typescript">
{"addingSpendInvariant(thingWithSeed: "}<a href="#hasSeed">{"hasSeed"}</a>{" | "}<a href="#TxOutputId">{"TxOutputId"}</a>{" | string"}{"): "}<a href="#isActivity">{"isActivity"}</a>{";"}
            </code></pre>
...
<div className="prose">
        <a id="CreatingNewSpendDelegate"></a>

            <div>
        <h4 style={{display: "inline-block"}}>    {
              "CreatingNewSpendDelegate"}()&nbsp;&nbsp;</h4>
        <ReactMarkdown>
            {
...
<p></p>
        ...
        <ReactMarkdown> {
            "\n\nSee the `tcxWithSeedUtxo()` method in your contract's off-chain StellarContracts subclass to create a context satisfying `hasSeed`. See `$seeded$CreatingNewSpendDelegate}` for use in a context providing an implicit seed utxo.\n"
        }</ReactMarkdown>
            <pre><code lang="typescript">
{"CreatingNewSpendDelegate(value: "}<a href="#hasSeed">{"hasSeed"}</a>{", fields: "}{"{\n        replacingUut: number[] | undefined;\n    }"}{"): "}<a href="#isActivity">{"isActivity"}</a>{";"}
            </code></pre>
...
<p></p>
            ...
            <pre><code lang="typescript">
{"CreatingNewSpendDelegate(fields: "}<a href="#MinterActivity$CreatingNewSpendDelegateLike">{"MinterActivity$CreatingNewSpendDelegateLike"}</a>{" | {\n        seed: "}<a href="#TxOutputId">{"TxOutputId"}</a>{" | string;\n        replacingUut: number[] | undefined;\n    }"}{"): "}<a href="#isActivity">{"isActivity"}</a>{";"}
            </code></pre>
...
<div className="prose">
        ...
        <ReactMarkdown>
            {
                "generates isActivity/redeemer wrapper with UplcData for ***\"CapoMintHelpers::MinterActivity.forcingNewMintDelegate\"***, given a transaction-context (or direct arg) with a ***seed utxo***\n\n"
            }
        </ReactMarkdown>
    </div>
...
<p></p>
        ...
        <ReactMarkdown> {
            "\n\n##### Seeded activity This activity uses the pattern of spending a utxo to provide a uniqueness seed. - to get a transaction context having the seed needed for this argument, see the `tcxWithSeedUtxo()` method in your contract's off-chain StellarContracts subclass. - or see the {@link hasSeed} type for other ways to feed it with a TxOutputId. - in a context providing an implicit seed utxo, use the `$seeded$forcingNewMintDelegate}` variant of this activity instead\n"
        }</ReactMarkdown>
            <pre><code lang="typescript">
{"forcingNewMintDelegate(thingWithSeed: "}<a href="#hasSeed">{"hasSeed"}</a>{" | "}<a href="#TxOutputId">{"TxOutputId"}</a>{" | string"}{"): "}<a href="#isActivity">{"isActivity"}</a>{";"}
            </code></pre>
...
<p></p>
            ...
            <pre><code lang="typescript">
{"mintingCharter(owner: "}<a href="#Address">{"Address"}</a>{" | string"}{"): "}<a href="#isActivity">{"isActivity"}</a>{";"}
            </code></pre>
