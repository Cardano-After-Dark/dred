...
export default function DocumentItem() {
    return (
        <div>
                <h2>HeliosProgramWithCacheAPI</h2>
                    <ReactMarkdown>
                        {
        ...
        <ReactMarkdown> {
            "\n\nWhen building through this interface in the browser, the async API is the same as that used in a development environment (using node.js). In the developer environment, the cache is a file system cache. In the browser, there is not current a cache implementation.\n\nOtherwise, the compileWithCache() is simply an async wrapper around the normal interface for compiling a Helios program with options including optimization.\n\n\n"
        }</ReactMarkdown>
      
            <p>
                <b>Static / class methods: </b>
                                <a href="#checkFile"><var>checkFile()</var></a>
            </p>
            <p>
                <b>Instance properties: </b>
                                <a href="#cacheEntry"><var>cacheEntry</var></a>
            </p>
         <p>
            <b>Instance methods: </b>
                    <a href="#compileWithCache"><var>compileWithCache()</var></a>
        </p>
...
{"static checkFile(srcFilename: "}{"string"}{"): "}{"boolean | null"}{";"}
    ...
    <div className="prose">
        ...
        <ReactMarkdown>
            ...
            <pre><code lang="typescript">
{"cacheEntry: "}<a href="#HeliosProgramCacheEntry">{"HeliosProgramCacheEntry"}</a>{" | undefined"}{";"}
            </code></pre>
    ...
    <h3>Instance methods</h3>
        ...
        <ReactMarkdown>
            ...
            <pre><code lang="typescript">
{"compileWithCache(optimizeOrOptions: "}{"boolean | "}<a href="#CompileOptionsForCachedHeliosProgram">{"CompileOptionsForCachedHeliosProgram"}</a>{"): "}<a href="#Promise">{"Promise"}</a>{"<"}<a href="#anyUplcProgram">{"anyUplcProgram"}</a>{">"}{";"}
            </code></pre>
