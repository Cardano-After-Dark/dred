...
import { providerSignal } from "../capoSignals.js";
import type { 
...
// Signal for storing heartbeat interval from settings
const heartbeatIntervalSignal = useSignal<bigint>(BigInt(72 * 60 * 60 * 1000)); // Default 72 hours
...
// Computed values for node statistics
const activeNodesSignal = useComputed(() => {
  return nodeRegistrySignal.value.filter((node: NodeRegistrationData) => 
    isNodeActive(node.lastHeartbeat, heartbeatIntervalSignal.value)
  ).length;
});

const lastUpdateSignal = useSignal<Date>(new Date());
  ...
  useEffect(() => {
    const fetchNodeRegistry = async () => {
      ...
      try {
        ...
        const charterData = await provider.findSettingsInfo({ 
          charterData: {} as CharterData,
          capoUtxos: [] as TxInput[]
        });
        if (charterData) {
          const settings = await provider.getSettingsController({ charterData });
          if (settings) {
            const protocolSettings = await settings.initialSettingsData();
            if (protocolSettings?.nodeOpSettings?.minHeartbeatInterval) {
              heartbeatIntervalSignal.value = BigInt(protocolSettings.nodeOpSettings.minHeartbeatInterval);
            }
          }
        }

        // Get node registry entries
        const nodeEntries = await provider.findNodeOpVaultEntries();
        if (nodeEntries) {
          ...
          nodeRegistrySignal.value = validNodes;
        }
        lastUpdateSignal.value = new Date();
      } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-2xl">{nodeRegistrySignal.value.length}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold mb-2">Active Nodes</h3>
          <p className="text-2xl">{activeNodesSignal.value}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold mb-2">Last Updated</h3>
          <p className="text-sm">{lastUpdateSignal.value.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {nodeRegistrySignal.value.map((node, index) => {
